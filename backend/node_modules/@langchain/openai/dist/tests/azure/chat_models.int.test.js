...
test("Test Azure ChatOpenAI with timeout in call options", async () => {
    const model = new AzureChatOpenAI({ maxTokens: 5 });
    await expect(() => model.invoke([new HumanMessage("Print hello world")], { timeout: 10 })).rejects.toThrow();
}, 5000);
...
test("Test Azure ChatOpenAI token usage reporting for streaming function calls", async () => {
    ...
    if (nonStreamingResult.additional_kwargs.function_call?.arguments &&
        ...
        console.log(`Function Call: ${JSON.stringify(nonStreamingResult.additional_kwargs.function_call)}`);
